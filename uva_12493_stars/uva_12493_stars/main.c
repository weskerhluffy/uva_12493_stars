...
COMUN_FUNC_STATICA int stars_core(entero_largo_sin_signo n, primos_datos *pd,
	...
	natural i = 0;
	for (i = 0; i < pd->primos_criba_tam; i++) {
		natural primo = pd->primos_criba[i];
//		comun_log_debug("primo %u contribucion primos mayores %llu", primo,
//				contribucion_primos_mayores);
		if (contribucion_primos_mayores > raiz_cubica) {
			break;
		}
//		comun_log_debug("primo %u", primo);
		stars_calcula_potencia_primo(factor_primos_mayores, primo,
				&factor_primos_mayores);
	}
	factor_primos_menores = n / factor_primos_mayores;
